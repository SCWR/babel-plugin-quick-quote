...
const defConfig = loadConfig('echarts')
...
function build (name, quote, { types: t }) {
  const {elements} = quote
  let importDefaultSpecifiers = name ? [t.importDefaultSpecifier(t.identifier(name))] : []
  return [
    t.importDeclaration(importDefaultSpecifiers, t.stringLiteral(ROOT_MODULES_PATH)),
    ...elements.map((element) => t.importDeclaration([], t.stringLiteral(getModulePath(element.value)))),
  ]
}

function buildAsync (name = DEF_FUN_NAME, quote, { template }, variableName) {
  const {elements} = quote
  const requiredModules = elements.map((element) => `import(/* webpackChunkName: "echarts" */ '${getModulePath(element.value)}')`)
    .join(',')
